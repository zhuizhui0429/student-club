...
import type { ActivityOfClub } from "@api";
...
export interface ActivityFormStateType
  extends Omit<ActivityOfClub, "poster" | "date"> {
  poster: File;
  description: string;
}
const formRef = ref<FormExpose>({} as any);
const formState = reactive<ActivityFormStateType>({} as any);
...
<template>
  <div class="publish_activity_container">
    <a-form
      ref="formRef"
      :model="formState"
      :rules="rules"
    ...
    >
      <a-form-item label="活动海报" name="poster">
        <AvatarUpload v-model:avatarFile="formState.poster" />
      </a-form-item>
