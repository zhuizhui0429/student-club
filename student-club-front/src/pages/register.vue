...
export default defineComponent({
  setup() {
    ...
    const validatePassword = async function (_rule: RuleObject, value: RegisterFormState['password']) {
    ...
    const validatePassword2 = async function (_rule: RuleObject, value: RegisterFormState['password2']) {
      if (value.length === 0) {
    ...
    const rules: Record<keyof RegisterFormState, RuleObject | RuleObject[]> = {
      type: [{ required: true, message: '请选择你的身份再进行注册', trigger: 'blur' }],
      account: [
        {
          min: 8,
          max: 16,
      ...
      password2: [{ required: true, validator: validatePassword2, trigger: 'change' }]
    }
      ...
      <div class="form_area">
        <a-form ref="formRef" :rules="rules" :model="formState" :label-col="labelCol" :wrapper-col="wrapperCol">
          <a-form-item has-feedback label="账号" name="account">
            <a-input v-model:value="formState.account" placeholder="请填写账号" />
          </a-form-item>
          <a-form-item has-feedback label="密码" name="password">
            <a-input-password v-model:value="formState.password" placeholder="请填写密码" />
          </a-form-item>
          <a-form-item has-feedback label="二次确认" name="password2">
            <a-input-password v-model:value="formState.password2" placeholder="请再次填写密码" />
          </a-form-item>
          <a-form-item label="用户类型" name="type">
            <a-select v-model:value="formState.type" placeholder="请选择你的注册身份">
              <a-select-option value="student">学生</a-select-option>
              <a-select-option value="manager">俱乐部经理</a-select-option>
            </a-select>
          </a-form-item>
          <div class="operation_area">
            <a-button size="large" type="primary" @click="register" :loading="isRegistering">注册</a-button>
            <a-button size="large" style="margin-left: 20px" type="dashed" @click="reset">清空</a-button>
          </div>
        </a-form>
      </div>
    </div>
